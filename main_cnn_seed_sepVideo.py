...
from io_utils import SEEDDataset_raw, TrainSampler_SEED
...
parser.add_argument('--batch-size-pretrain', default=9, type=int, metavar='N',
                    help='mini-batch size')
parser.add_argument('--batch-size-pretrain-val', default=6, type=int, metavar='N',
                    help='mini-batch size')
...
data_train_all, data_val_all, label_train_repeat, label_val_repeat, n_samples, n_samples_remain = load_seed_raw_trainVal(data_dir, timeLen, timeStep, channel_norm)
...
args.log_dir = 'sepVideo_raw_batch%d_timeLen%d_tf%d_sf%d_tfLen%d_multiFact%d_lr%f_wd%f_epochs%d_randSeed%d_accSel' % (
...
for fold in range(n_folds):
    ...
    val_sampler = TrainSampler_SEED(len(val_sub), n_times=args.n_times, batch_size=args.batch_size_pretrain_val,
