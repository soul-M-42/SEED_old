...
class SEEDDataset_raw(Dataset):
    def __init__(self, data, label, timeLen, timeStep, n_samples, n_samples_remain, fs, transform=None):
        ...
        n_subs = int(len(self.label) / np.sum(n_samples))
        n_samples_all = np.tile(n_samples, n_subs)
        self.n_samples_cum = np.concatenate((np.array([0]), np.cumsum(n_samples_all)))
        n_samples_remain_all = np.tile(n_samples_remain, n_subs)
        self.n_samples_remain_cum = np.concatenate((np.array([0]), np.cumsum(n_samples_remain_all)))
    ...
    def __getitem__(self, idx):
        ...
        one_seq = self.data[:, int((idx * self.timeStep + self.n_samples_remain_cum[pass_videos]) * self.fs):
                               int((idx * self.timeStep + self.timeLen + self.n_samples_remain_cum[pass_videos]) * self.fs)]
        one_label = self.label[idx]
...
class TrainSampler_SEED():
    def __init__(self, n_subs, n_times, batch_size, n_samples):
        ...
        for i in range(self.n_subs):
            # j = i
            for j in range(i+3, self.n_subs, 3):
                self.sub_pairs.append([i, j])
        # print(self.sub_pairs[:3])
