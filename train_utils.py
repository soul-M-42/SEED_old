...
def train_earlyStopping(args, train_loader, val_loader, model, criterion, optimizer, scheduler):
    bad_count = 0
    ...
    for epoch in range(args.epochs_finetune):
        ...
        for counter, (x_batch, y_batch) in enumerate(val_loader):
            ...
            top1 = accuracy(logits, y_batch, topk=(1,))
            confusionMat = confusionMat + get_confusionMat(logits, y_batch, 3)
            val_acc += top1[0]
            val_loss += loss.data.cpu().numpy()
